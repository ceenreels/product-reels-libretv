(function (root) {
  ...
  function getEligibleSources(opts) {
    ...
    ids = ids.filter(function(id,i){ var s=info(id,map[id],i); return s.enabled && s.defaultEligible && isSourceHealthy(id, opts.now) && s.capabilities.search !== false && langMatch(s,opts.locale) && (!s.regions || s.regions.indexOf(opts.region)>=0 || s.regions.indexOf('GLOBAL_EN')>=0 || s.regions.indexOf('GLOBAL_ZH')>=0); });
  ...
  function getFallbackSources(opts) {
    opts=opts||{}; var map=sourceMap(), eligible=getEligibleSources({locale:opts.locale,region:opts.region,mode:'aggregated',now:opts.now}), all=Object.keys(map).filter(function(id){var s=info(id,map[id],0); return s.enabled&&s.defaultEligible&&isSourceHealthy(id, opts.now)&&s.capabilities.search!==false;});
